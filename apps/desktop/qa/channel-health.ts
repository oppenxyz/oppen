...
export function channelHealthFixture(binding: ChartBinding, revision = "1", scenario = "connected"): ChannelHealthSnapshot {
  ...
  const snapshot: ChannelHealthSnapshot = {
    ...
    rows: channels.map((channel, index) => ({
      owner: index < 3 ? "console" : "chart", channel, connection_id: "1", subscribed: true,
      ...
      consumer_failure: scenario === "failure" && index >= 3 ? "Synthetic consumer failed <script>not executable</script> " + "diagnostic/".repeat(20) : null,
