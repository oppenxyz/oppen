...
| ES2 | Unknown outcomes | Keep the submitted cloid reserved until an authoritative order-status result; never release on `unknownOid` | A missing result is not proof that the exchange did not apply the order. ES8 persists the reservation across dropped requests and process restarts; the complete exchange/restart gate remains open. |
...
| ES4 | Exposure arithmetic | Keep opening and reduce-only sizes by side; compare independently reachable fill outcomes; value each symbol's opening orders at its quoted reference mark | Opposite orders do not necessarily fill together. A clipped reduction can flatten before an opening order flips the position. Replace a symbol's resting contribution using the same valuation that was added to the account total. Limit prices can understate cross-symbol exposure; missing or nonpositive marks invalidate the whole resting valuation instead of publishing partial headroom. Order JSON keeps its limit price; the internal mark is not serialized. |
...
| W2 | What moves the chart | **Venue-preferred candles, separate partial tape bars and a latest-trade marker** (revised 2026-09-08; implemented locally) | The original optimistic tape extension followed by venue overwrite could double-count prints already included in cumulative candle volume. Bucket timestamps do not establish inclusion or cross-channel chronology. Rust owns the source-separated projection: prefer a venue bar, otherwise show a partial locally observed bar; never blend their OHLCV. A separate timestamp-ordered trade marker preserves visible tape activity without pretending to update the venue close. Equal-time ambiguity, interrupted observation and bounded dedup freezes remain explicit. A fresh, serially drained chart-only transport incarnation prevents delayed A-B-A frames from entering a new selection while account supervision remains unchanged. Reuse the existing pool and core interval/bar validation; no new dependency. **Tradeoff:** one bounded additional read-only chart connection and no optimistic modification of venue candles. The [live chart contract](specs/live-chart-observations.md) defines ownership, REST merges, limits and local evidence; remote CI, installed and public-feed acceptance remain pending. |
| W3 | What streams and what polls | **The selected symbol streams; the rail still polls every 10 s** (rationale corrected 2026-09-08) | The original claim that Hyperliquid has no aggregate-market channel is obsolete. Its [subscription reference](https://hyperliquid.gitbook.io/hyperliquid-docs/for-developers/api/websocket/subscriptions) lists aggregate candidates including `allMids` and `allDexsAssetCtxs`. Oppen has not adopted those adapters; selected context, BBO, depth, trades and candles remain the implemented public streams. The rail poll is current behavior, not a venue limitation. Adoption follows channel-health correction with typed fixtures, symbol/universe mapping and explicit partial-field semantics. **What this gives up:** other rail rows and volume ordering can remain ten seconds old. New data does not imply re-sorting rows under the operator's pointer on every tick. |
| W4 | Who owns market-channel observations | **Native pool facts, distinct observation ages and displayed quote validity** (revised 2026-09-08; implementation in progress) | The previous `feedTick` aggregate allowed context traffic, reconnect and quarantine notices to report all market channels OK. It also mixed venue timestamps with host time. Reuse the pool's subscription-specific connection, acknowledgment, quarantine and receipt facts through nonblocking runtime status sampling. A separate five-second monotonic UI deadline applies only to accepted health snapshots; successful null polls cannot renew it. Existing reference-age budgets stay unchanged and do not claim event-driven BBO silence is a disconnect. Quote/bar reducers retain their own values and observation clocks; health is not execution eligibility or reconciliation. The [channel-health contract](specs/console-channel-health.md) defines ownership, diagnostics, known quote A-B-A limits and acceptance. No subscription, signing guard or account supervisor is replaced by this decision. |
